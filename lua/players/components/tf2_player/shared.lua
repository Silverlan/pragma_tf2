...
Component:RegisterMember(
	"DefaultForwardMovementSpeedFactor",
	ents.MEMBER_TYPE_FLOAT,
	1.0,
	{},
	ents.BaseEntityComponent.MEMBER_FLAG_DEFAULT_TRANSMIT
)
Component:RegisterMember(
	"DefaultBackwardMovementSpeedFactor",
	ents.MEMBER_TYPE_FLOAT,
	0.9,
	{},
	ents.BaseEntityComponent.MEMBER_FLAG_DEFAULT_TRANSMIT
...
Component:RegisterMember(
	"DefaultCrouchedMovementSpeedFactor",
	ents.MEMBER_TYPE_FLOAT,
	0.33,
	{},
	ents.BaseEntityComponent.MEMBER_FLAG_DEFAULT_TRANSMIT
...
function Component:UpdateMovementSpeed()
	...
	if charComponent ~= nil then
		charComponent:SetJumpPower(self:GetDefaultJumpHeight())
	end
end

function Component:TranslateActivity(act)
	if TF2_ACTIVITIES[act] == nil then
		return
	end
...
function Component:InitializePlayer(plC)
	...
	if charComponent ~= nil then
		charComponent:SetMoveController("move_x", "move_y")
	end
...
function Component:ClearClass()
	if self.m_class ~= tf2.PLAYER_CLASS_INVALID then
		self:GetEntity():RemoveComponent(tf2.player_class_to_identifier(self.m_class))
	end
	if CLIENT then
		self:ResetViewModel()
	end
...
ents.tf2.COMPONENT_PLAYER =
	ents.register_component("tf2_player", Component, ents.EntityComponent.FREGISTER_BIT_NETWORKED)
Component.NET_EVENT_CHANGE_CLASS = ents.register_component_net_event(ents.tf2.COMPONENT_PLAYER, "change_class")
Component.EVENT_APPLY_CHARACTER_DIMENSIONS =
	ents.register_component_event(ents.tf2.COMPONENT_PLAYER, "apply_character_dimensions")
Component.EVENT_APPLY_CHARACTER_MODEL =
	ents.register_component_event(ents.tf2.COMPONENT_PLAYER, "apply_character_model")
Component.EVENT_APPLY_CHARACTER_LOADOUT =
	ents.register_component_event(ents.tf2.COMPONENT_PLAYER, "apply_character_loadout")
