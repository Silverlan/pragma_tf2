...
function Component:Initialize()
	...
	if(pickupC ~= nil) then pickupC:SetModel("items/medkit_small") end
	if(SERVER) then
		local healthkitC = self:AddEntityComponent("tf2_healthkit")
		if(healthkitC ~= nil) then healthkitC:SetRestoreAmount(0.205) end
	end
end
ents.tf2.COMPONENT_HEALTHKIT_SMALL = ents.register_component("tf2_healthkit_small",Component)
