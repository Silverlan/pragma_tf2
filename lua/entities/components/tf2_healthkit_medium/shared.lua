...
function Component:Initialize()
	...
	if(pickupC ~= nil) then pickupC:SetModel("items/medkit_medium") end
	if(SERVER) then
		local healthkitC = self:AddEntityComponent("tf2_healthkit")
		if(healthkitC ~= nil) then healthkitC:SetRestoreAmount(0.5) end
	end
end
ents.tf2.COMPONENT_HEALTHKIT_MEDIUM = ents.register_component("tf2_healthkit_medium",Component)
