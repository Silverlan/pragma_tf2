...
function Component:HandleKeyValue(key, val)
	if key == "associatedmodel" then
		self.m_associatedModel = val
	else
...
function Component:OnStartTouch()
	if self.m_state ~= STATE_READY_FOR_USE then
		return
	end
	local ent = self:FindAssociatedModelEntity()
	if ent == nil then
...
function Component:FindAssociatedModelEntity()
	return (self.m_associatedModel ~= nil) and ents.iterator({ ents.IteratorFilterName(self.m_associatedModel) })()
		or nil
end
