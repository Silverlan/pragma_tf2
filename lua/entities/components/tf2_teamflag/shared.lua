...
function Component:Initialize()
	...
	if(pickupC ~= nil) then
		...
		if(CLIENT) then pickupC:SetIdleAnimation("spin") end
	end

	self:BindEvent(ents.OwnableComponent.EVENT_ON_OWNER_CHANGED,"OnOwnerChanged")
	if(SERVER) then self:BindEvent(ents.tf2.ItemPickup.EVENT_ON_PICKED_UP,"OnPickedUp") end
end
function Component:OnOwnerChanged(prevOwner,owner)
	...
	if(util.is_valid(owner)) then
		...
		attInfo.flags = bit.bor(attInfo.flags,ents.AttachableComponent.FATTACHMENT_MODE_SNAP_TO_ORIGIN,ents.AttachableComponent.FATTACHMENT_MODE_UPDATE_EACH_FRAME,ents.AttachableComponent.FATTACHMENT_MODE_FORCE_IN_PLACE)
		attC:AttachToAttachment(owner,"flag",attInfo)
		if(CLIENT) then ent:PlayAnimation("idle") end
		if(physC ~= nil) then physC:SetSimulationEnabled(false) end
		return
	end
	local attC = ent:GetComponent(ents.COMPONENT_ATTACHABLE)
	if(attC ~= nil) then attC:ClearAttachment() end
	if(CLIENT) then ent:PlayAnimation("spin") end
	if(physC ~= nil) then
...
function Component:OnEntitySpawn()
	...
	if(entGm ~= nil) then
		...
		if(componentName ~= nil) then entGm:AddComponent(componentName) end
	end
end
