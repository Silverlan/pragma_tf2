...
function Component:GetMatchSummary() return self.m_matchSummary or 0 end
function Component:HandleKeyValue(key,val)
	if(key == "matchsummary") then self.m_matchSummary = toint(val)
	else return util.EVENT_REPLY_UNHANDLED end
	return util.EVENT_REPLY_HANDLED
end
ents.tf2.COMPONENT_PLAYER_TEAM_SPAWN = ents.register_component("tf2_player_teamspawn",Component)
